...
#define DELETE_RESOURCE(XX) \
  ...
  void RpcDispatcher::dispatch(AbstractProtocol::s_ptr request, AbstractProtocol::s_ptr response, TcpConnection *connection)
  {
    ...
    RpcController *rpc_controller = new RpcController(); // 实例化一个RPCController对象

    rpc_controller->SetLocalAddr(connection->getLocalAddr()); // 获取本地地址
    rpc_controller->SetPeerAddr(connection->getPeerAddr());   // 获取对端地址
